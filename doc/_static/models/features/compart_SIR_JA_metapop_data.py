            ...
            for source in moves[self.statevars.step]:
                for dest, age, qty in moves[self.statevars.step][source]:
                    ...
                    if source not in herds:
                        ...
                        # specify characteristics of new population to comply with movement
                        new_population = [(age, qty, prototype)]
                        if dest not in herds:
                            pass
                        else:
                            herds[dest].add_new_population(new_population)
                            herds[dest].statevars.purchased += qty

                    else:
                        # find convenient animals
                        removed_population = herds[source].remove_randomly(amount={(age,): qty})
                        total_amount = sum(removed_population.values())
                        herds[source].statevars.sold += total_amount
                        if dest not in herds:
                            pass
                        # print('movement from {} going outside the metapopulation'.format(source))
                        else:
                            herds[dest].add_population(removed_population)
                            # update variable 'purchased' in dest herd
                            herds[dest].statevars.purchased += total_amount
